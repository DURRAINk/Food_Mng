...
connection_string = (
    "mssql+pyodbc://LAPTOP-O4M1T2S4\\SQLEXPRESS/Food_Management"
    "?driver=SQL+Server"
...
sql_queries = ["select City, count(Name) as Provider_Count from providers group by City", "select City, count(Name) as Receiver_Count from receivers group by City",
           "select Type,count(Name)as Total from providers group by Type order by Total desc;",
            ...
            "select Food_Type, count(Meal_Type) as Count from food_listings group by Food_Type order by Count desc;","select Food_Name, count(Claim_ID) as Total_Claims from (select fl.Food_Name, cm.Claim_ID from food_listings fl join claims cm on fl.Food_ID = cm.Food_ID) as tables group by Food_Name order by Total_Claims desc;", "select top 1 Name, count(Status) as Highest_Claims from (select pd.Name, cm.Status from providers pd join food_listings fl on pd.Provider_ID =fl.Provider_ID join claims cm on fl.Food_ID = cm.Food_ID where cm.Status = 'Completed') as tables group by Name order by Highest_Claims desc;", "select Status, cast(Count(Claim_ID) as float)*100 /(select count(*) from claims) as [Status%] from Claims group by Status;", "select rv.Name as Receiver_Name, avg(fl.Quantity) as Avg_Quantity from receivers rv join claims cm on rv.Receiver_ID = cm.Receiver_ID join food_listings fl on fl.Food_ID = cm.Food_ID group by rv.Name order by Avg_Quantity desc;", "select fl.Meal_Type, count(cm.Status) as Count from claims cm join food_listings fl on cm.Food_ID = fl.Food_ID group by fl.Meal_Type order by Count desc;", "select pd.Name , sum(fl.Quantity) as Total_Qunatity from providers pd join food_listings fl on pd.Provider_ID = fl.Provider_ID group by pd.Name order by Total_Qunatity desc;","select fl.Food_ID, fl.Food_Name, cm.Timestamp as Claim_Date, fl.Expiry_Date from claims cm join food_listings fl on cm.Food_ID = fl.Food_ID","select rc.Type as Receiver_Type , sum(fl.Quantity) as Total_Quantity from receivers rc join claims cm on rc.Receiver_ID = cm.Receiver_ID join food_listings as fl on cm.Food_ID = fl.Food_ID group by rc.Type order by Total_Quantity desc;"]
